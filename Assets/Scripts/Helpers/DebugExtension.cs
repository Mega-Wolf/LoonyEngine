...
// from https://assetstore.unity.com/packages/tools/debug-drawing-extension-11396
...
public static class DebugExtension {
	...
	public static void DrawPoint(Vector3 position, Color color, float scale = 1.0f, float duration = 0) {
		...
		if (duration == 0) {
			Debug.DrawLine(position + (Vector3.up * (scale * 0.5f)), position + (Vector3.down * (scale * 0.5f)), color);
			Debug.DrawLine(position + (Vector3.left * (scale * 0.5f)), position + (Vector3.right * (scale * 0.5f)), color);
		} else {
			Debug.DrawLine(position + (Vector3.up * (scale * 0.5f)), position + (Vector3.down * (scale * 0.5f)), color, duration);
			Debug.DrawLine(position + (Vector3.left * (scale * 0.5f)), position + (Vector3.right * (scale * 0.5f)), color, duration);
		}
